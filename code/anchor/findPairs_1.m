function [anchors, listing, numClust] = findPairs_1(iso_3166_2,inDir,outHash,varargin)
...
% Required
addRequired(p,'iso_3166_2',@ischar);
addRequired(p,'inDir',@iscell);
addRequired(p,'outHash',@ischar);

% Optional
addParameter(p,'anchorRange_nm',1.00,@isnumeric);
addParameter(p,'classInclude',{'B','C','D','O'},@iscell);
addParameter(p,'fileAirspace',[getenv('AEM_DIR_CORE') filesep 'output' filesep 'airspace.mat'],@ischar);
addParameter(p,'spheroid',wgs84Ellipsoid('nm'));
addParameter(p,'seed',double.empty(0,0),@isnumeric);
addParameter(p,'isSave',true,@islogical);

% Parse
parse(p,iso_3166_2,inDir,outHash,varargin{:});
...
%% Preallocate, calculate grid, and load airspace
[anchors, gridLat_deg, gridLon_deg, airspace] = preallocAnchors(iso_3166_2, anchorRange_nm, p.Results.fileAirspace, false);
...
listing = dir([inDir{1} filesep '**' filesep '*.csv']);

% Concat listings if there is more than one input directory
if numel(inDir) > 1
    for i=2:1:numel(inDir)
        listing = [listing; dir([inDir{i} filesep '**' filesep '*.csv'])];
    end
end

if isempty(listing); warning('RUN_1:listing','No files found inDir, exiting via RETURN'); return; end
...
for k=1:1:numClust
    ...
    parfor i=1:1:numel(listk)
        % Load trajectory
        TTout = textscantraj([listk(i).folder filesep listk(i).name]);
        
        % Parse
        time_s{i} = seconds(TTout.Time);
        lat_deg{i} = TTout.lat_deg;
        lon_deg{i} = TTout.lon_deg;
